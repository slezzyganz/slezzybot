...
//Modal Dikit Broh:v
LeysKey = 'YOUR_APIKEY' //beli di https://leyscoders-api.herokuapp.com
LolKey = 'YOUR_APIKEY' //beli di https://api.lolhuman.xyz

//Settings => Semua Aturnya Disini <=
prefix = '/' //Ubah jadi sesuka lu
fake = 'EITS ADA ANTIDELETE🐦:v'
namabot = 'BotRihan' //Ubah jadi nama bot lu
namaowner = 'R' //Ubah jadi nama lu
nomerowner = '+6289502228065' //Ubah jadi nomer lu (pake kode negara)
...
dp.on('message-new', async (dap) => {
	try {
		...
		'🍌 : 🍌 : 🍌'
		    ...
		    const R = (pesan, tipe, target, target2) => {
...
//DASAR MENU
				...
				dp.sendMessage(`${nomerowner}@s.whatsapp.net`, options, text, {quoted: fkontak})
...
//MENU
                ...
                const RAM = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
                const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model, device_network, cpu_device, device_battery, device_ram } = dp.user.phone
                dapuhy = `
╭────ꕥ ${a}Info Bot${a} ꕥ────────✾
│❒ ${a}𝖭𝖺𝗆𝖺 : ${namabot}${a}
│❒ ${a}Server : ${dp.browserDescription[0]}${a}
│❒ ${a}Ram : ${RAM}${a}
│❒ ${a}MCC : ${mcc}${a} 
│❒ ${a}MNC : ${mnc}${a} 
